...
print('\n\n')
print('Test LazyDreamSimMetric')
dreamsim_metric = metric.LazyDreamSimMetric(dataset, device)
print("Output shape (should be 2 x 3): ", dreamsim_metric.compute_similarity(ind_a, ind_b).shape)
print(f"Perceptual similarity (should be symmetric and 0 on diagonal): {dreamsim_metric.compute_similarity(ind_a, ind_a)}")
print(f"Same result? (i.e. test for deterministic): {metric.LazyDreamSimMetric(dataset, device).compute_similarity(ind_a, ind_a)}")